...
public class ImageMatrixGUI extends Observable {
	...
	private class RogueWindow extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			synchronized (images) { // Added 16-Mar-2016
				...
				if(imageToFillRogueWindow != null)
					g.drawImage(imageDB.get(imageToFillRogueWindow.getName()).getImage(),0,0,
							SQUARE_SIZE*N_SQUARES_WIDTH,SQUARE_SIZE*N_SQUARES_HEIGHT, frame);
			}
		}
	}

	private ImageTile imageToFillRogueWindow = null;

	public void setImageToFillRogueWindow(ImageTile imageToFillRogueWindow){
		if (imageToFillRogueWindow == null) {
			this.imageToFillRogueWindow = null;
			return;
		}
		if (!imageDB.containsKey(imageToFillRogueWindow.getName())) {
			throw new IllegalArgumentException("No such image in DB " + imageToFillRogueWindow.getName());
		}
		this.imageToFillRogueWindow = imageToFillRogueWindow;
	}
	...
	private class InfoWindow extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			if(imageToFillInfoWindow != null)
				g.drawImage(imageDB.get(imageToFillInfoWindow.getName()).getImage(),0,0,
						SQUARE_SIZE*N_SQUARES_WIDTH,SQUARE_SIZE*N_SQUARES_HEIGHT, frame);
		}
	}

	private ImageTile imageToFillInfoWindow = null;

	public void setImageToFillInfoWindow(ImageTile imageToFillInfoWindow){
		if (imageToFillInfoWindow == null) {
			this.imageToFillInfoWindow = null;
			return;
		}
		if (!imageDB.containsKey(imageToFillInfoWindow.getName())) {
			throw new IllegalArgumentException("No such image in DB " + imageToFillInfoWindow .getName());
		}
		this.imageToFillInfoWindow  = imageToFillInfoWindow ;
	}
